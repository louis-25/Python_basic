...
# WHERE Retrieve3
c.execute('SELECT * FROM users WHERE id=:Id' ,{"Id": 5}) # 5번 id만 가져온다
...
# WHERE Retrieve5 - tuple
c.execute("SELECT * FROM users WHERE id IN('%d','%d')" % (3,4))
print('param5',c.fetchone())

# WHERE Retrieve6 - dictionary
c.execute("SELECT * FROM users WHERE id=:id1 OR id=:id2", {"id1": 2, "id2": 5})
print('param6',c.fetchone())

# Dump 출력
with conn: # with문을 사용하면 close를 별도로 선언하지 않아도 된다
    with open('D:\Study\IT_Skill\패스트캠퍼스\Python\python_basic/resource/dump.sql', 'w') as f:
